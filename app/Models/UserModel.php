...
class UserModel extends Model {
    ...
    public function getJuryRateData($jury_id = '',$nominee_id = '')
    {
        ...
        $builder->where('YEAR(ratings.created_date)',date('Y'));
        return $query = $builder->get();
    }
    ...
    public function getListsNominations($jury_id='')
    {
       ...
       // $builder->where("nominee_details.nomination_year",'');
	$builder->where("users.award_id",22);
    ...
    public function getAllActiveJuryListsToMail($main_category_id=array())
    {
        $builder = $this->table('users');
        $builder->select('users.*,jury_mapping.award_id as jury_mapped_award_id');
	$builder->join('jury_mapping','jury_mapping.jury_id = users.id');
        $builder->where("users.role",'1');
        $builder->where("users.active",'1');
	$builder->whereIn('jury_mapping.award_id',$main_category_id);
        return $query = $builder->get();
    }
