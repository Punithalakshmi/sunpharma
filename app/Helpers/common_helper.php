...
if(!function_exists('getNominationDays'))
{
    function getNominationDays($extendDate) {
        ...
        $date2_ts = date("Y-m-d 23:59:59", strtotime($extendDate));
	...
	$nominationModel = model('App\Models\NominationModel');
       ...
       // }
		 $userModel->orderBy('id', 'DESC');
                ...
                if(is_array($awardData) && count($awardData)>0){ 
                    $nomineeData =  $nominationModel->getWhere(array("nominee_id" => $awardData['id']))->getRowArray();
                    //get registration_no
                    $registrationNo = (isset($nomineeData['registration_no']) && !empty($nomineeData['registration_no']))?$nomineeData['registration_no']:'';			
...
if(!function_exists('copyDocumentsFromLastYear')){
    
    function copyDocumentsFromLastYear($src, $dst) {  
